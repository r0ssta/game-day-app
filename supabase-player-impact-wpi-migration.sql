-- Weighted Performance Index (WPI): persist opponent strength on matches and
-- weight each player's on-pitch net shot differential by opponent quality
-- (better 1.5×, equal 1.0×, lesser 0.5×) and the coach's 1–5 match review
-- rating (÷3 so a 3 is baseline). Null strength counts as equal.
...
begin
  if exists (
    ...
    where table_schema = 'public'
      ...
      and column_name = 'qualitative_context'
  ) then
    update public.matches
    set opponent_strength = case
      when coalesce(
        qualitative_context->>'opponentStrength',
        qualitative_context->>'opponentTier',
        qualitative_context->>'oppositionStrength'
      ) in ('better', 'tier3', 'superior') then 'better'
      when coalesce(
        qualitative_context->>'opponentStrength',
        qualitative_context->>'opponentTier',
        qualitative_context->>'oppositionStrength'
      ) in ('lesser', 'tier1', 'easy') then 'lesser'
      when coalesce(
        qualitative_context->>'opponentStrength',
        qualitative_context->>'opponentTier',
        qualitative_context->>'oppositionStrength'
      ) in ('equal', 'tier2', 'competitive') then 'equal'
      else opponent_strength
    end
    where opponent_strength is null
      and qualitative_context is not null;
  end if;
end $$;
...
begin
  ...
  match_weighted as (
    select
      ...
      mra.coach_rating,
      (
        (mfs.team_shots - mfs.opponent_shots)
