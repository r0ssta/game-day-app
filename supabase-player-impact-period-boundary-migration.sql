-- Period boundaries in calculate_player_impact must match recap/hub logic:
-- a tagged starting_lineup after play, or a kickoff sub_in when the clock
-- resets to 0. Mid-period clock rewinds (e.g. 817 → 173) are not new periods.
...
begin
  ...
  player_totals as (
    select
      ist.player_id,
      sm.team_id,
      count(distinct ist.match_id)::integer as matches_played,
