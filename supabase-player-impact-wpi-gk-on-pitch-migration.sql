-- Count goals, shots, and corners during goalkeeper stints in on-pitch +/- and WPI.
-- Minutes stay split field vs GK. Glove line still shows GK GA and saves.
...
begin
  ...
  match_field_stats as (
    select
      ...
      coalesce(sum(ist.team_corners), 0)::integer as team_corners,
      coalesce(sum(ist.opponent_corners), 0)::integer as opponent_corners,
  ...
  personal_assists as (
    select
      credited.match_id,
      credited.player_id,
      count(*)::integer as player_assists
    from (
      select e.match_id, e.assist_player_id as player_id
      from public.match_events e
      join scoped_matches sm on sm.id = e.match_id
      where e.event_type = 'goal'
    ...
    group by credited.match_id, credited.player_id
  ),
  match_team_goals as (
    select
      sm.id as match_id,
      greatest(
        coalesce(
          nullif(count(e.id) filter (where e.event_type = 'goal'), 0),
          nullif(sm.home_score, 0),
          1
        ),
        1
      )::numeric as match_team_goals
    from scoped_matches sm
    left join public.match_events e
      on e.match_id = sm.id
      and e.event_type = 'goal'
    group by sm.id, sm.home_score
  ),
  ...
  match_weights as (
    select
      ...
      coalesce(pa.player_assists, 0) as player_assists,
      sqrt(3.0 / mtg.match_team_goals) as scoreline_factor,
      (
    ...
    left join personal_goals pg
      on pg.match_id = mfs.match_id
      and pg.player_id = mfs.player_id
    left join personal_assists pa
      on pa.match_id = mfs.match_id
      and pa.player_id = mfs.player_id
    join match_team_goals mtg on mtg.match_id = mfs.match_id
  ),
  match_weighted as (
    select
      mw.*,
      (
        ...
        + mw.team_shots
        + mw.team_corners
        -- WPI_GOAL_BONUS = 2, WPI_ASSIST_BONUS = 1, times sqrt(3 / match_team_goals)
        + (2 * mw.player_goals + mw.player_assists) * mw.scoreline_factor
      ) * mw.wpi_weight as weighted_offense,
  ...
  player_totals as (
    select
      ...
      coalesce(sum(mw.player_goals), 0)::integer as player_goals,
      coalesce(sum(mw.player_assists), 0)::integer as player_assists,
  ...
  select
    ...
    coalesce(pt.player_goals, 0)::integer as player_goals,
    coalesce(pt.player_assists, 0)::integer as player_assists,
